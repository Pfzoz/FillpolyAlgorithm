...
ColorWheel *color_wheel, *bg_color_wheel, *veditor_color_wheel;
LightBar *light_bar, *bg_light_bar, *veditor_lightbar;
Canvas *canvas;
...
void veditor_lightbar_onclick(int x, int y, bool hit)
{
    if (hit)
    {
        veditor_lightbar->set_luminance(x, y);
        veditor_color_wheel->set_luminance((float)veditor_lightbar->get_luminance()/255);
    }
}
...
void canvas_onclick(int x, int y, bool hit)
{
    ...
    {
        veditor.unselect();
        veditor_color_wheel->set_visible(false);
        veditor_lightbar->set_visible(false);
    }
}
...
void handle_on_v_color_click(int x, int y, bool hit)
{
    int red = -1, blue = -1, green = -1;
    if (hit)
    {
        veditor_color_wheel->get_color(x, y, &red, &green, &blue);
        if (red != -1 && blue != -1 && green != -1)
        {
            veditor.red->update_text(std::to_string(red));
            veditor.green->update_text(std::to_string(green));
            veditor.blue->update_text(std::to_string(blue));
            veditor.update_red();
            veditor.update_green();
            veditor.update_blue();
            canvas->set_colors();
...
int main(int argc, char *args[])
{
    ...
    veditor_color_wheel = new ColorWheel(veditor.geometry.w, veditor.geometry.w, veditor.geometry.w / 4);
    veditor_color_wheel->set_visible(false);
    bg_color_wheel->set_visible(false);
    light_bar = new LightBar(2 * VW, color_wheel->geometry.h);
    bg_light_bar = new LightBar(2 * VW, color_wheel->geometry.h);
    bg_light_bar->set_visible(false);
    veditor_lightbar = new LightBar(2 * VW, veditor_color_wheel->geometry.h);
    veditor_lightbar->set_visible(false);
    ...
    //
    veditor_color_wheel->z_index = 1;
    veditor_lightbar->z_index = 1;
    // Canvas
    ...
    veditor_color_wheel->set_onclick(handle_on_v_color_click);
    veditor_lightbar->set_onclick(veditor_lightbar_onclick);
    /* Add Components */
    ...
    main_scene->add_component(veditor_color_wheel);
    main_scene->add_component(veditor_lightbar);
